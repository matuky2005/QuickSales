...
const ProductsPage = () => {
  ...
  const exportCsv = () => {
    const headers = ["id", "descripcion", "marca", "atributos", "precioSugerido", "moneda"];
  ...
  const exportTemplateCsv = () => {
    const headers = ["descripcion", "marca", "atributos", "precioSugerido", "moneda"];
  ...
  const importCsv = async (event) => {
    ...
    try {
      ...
      const monedaIndex = findIndex("moneda");
      ...
      for (const line of lines.slice(1)) {
        ...
        items.push({
          ...
          moneda: values[monedaIndex]?.trim()?.toUpperCase() === "USD" ? "USD" : "ARS"
        });
      }
      if (!items.length) {
      ...
      <div className="table-wrapper" style={{ marginTop: 16 }}>
        <table className="table">
          ...
          <tbody>
            ...
            {products.length === 0 && (
              <tr>
                <td colSpan="7" className="helper">Sin productos para mostrar.</td>
              </tr>
            )}
          </tbody>
        </table>
        ...
        {Object.entries(groupedByBrand).map(([brand, items]) => (
          <div key={brand}>
            ...
            <ul>
              {items.map((product) => (
                <li key={product._id}>
                  {product.descripcion} - {product.precioSugerido} {product.moneda || "ARS"}
