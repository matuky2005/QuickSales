...
export const createSale = async (req, res, next) => {
  try {
    ...
    const normalizedItems = items.map((item) => {
      ...
      const moneda = normalizeText(item.moneda || "ARS").toUpperCase() === "USD" ? "USD" : "ARS";
      const precioUnitarioOriginal =
        item.precioUnitarioOriginal !== undefined ? Number(item.precioUnitarioOriginal) : undefined;
      const tipoCambio = item.tipoCambio !== undefined ? Number(item.tipoCambio) : undefined;
      return {
        ...item,
        descripcionSnapshot,
        marca,
    ...
    for (const item of normalizedItems) {
      ...
      if (item.moneda === "USD" && (!item.precioUnitarioOriginal || !item.tipoCambio)) {
        return res.status(400).json({ message: "usd items require precioUnitarioOriginal and tipoCambio" });
      }
    }
    ...
    for (const item of normalizedItems) {
      ...
      if (!product) {
        product = await Product.create({
          ...
          precioSugerido:
            item.moneda === "USD" && item.precioUnitarioOriginal
        ...
        product.precioSugerido =
          item.moneda === "USD" && item.precioUnitarioOriginal
            ? item.precioUnitarioOriginal
            : item.precioUnitario;
        product.moneda = item.moneda || product.moneda || "ARS";
        await product.save();
      }
      mappedItems.push({
        ...item,
